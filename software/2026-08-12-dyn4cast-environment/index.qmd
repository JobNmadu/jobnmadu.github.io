---
title: "Dyn4cast Environment"
description: "An introduction to Dyn4cast packge and the various functions it presents."
author:
  - name: Job Nmadu
...
date: 2026-08-12
categories: [R, Dynamic forecasting, Machine Learning]
citation:
  url: https://jobnmadu.github.io/software/2026-08-12-dyn4cast-environment/
slug: dyn4cast-environment
image: logo.png
draft: false
---
...
The released version of _Dyn4cast_ can be installed from [CRAN](https://cran.r-project.org/web/packages/Dyn4cast/index.html). The canonical form for  [CRAN](https://cran.r-project.org/web/packages/Dyn4cast/index.html) is:

```
install.packages("Dyn4cast")
```

The development version is available now and can be installed from [GitHub](https://github.com/JobNmadu/Dyn4cast) with:
...
The development version can also be installed through [r-universe](https://jobnmadu.r-universe.dev/Dyn4cast). Use the form:
...
```
DynamicForecast(Data, date, series, dyrima, Trend, Type,
...
dyrima **ARIMA** object of the `series` obtained from `auto.rima` in
 forecast package.
...
# mdi: Sequential Computation of Dynamic Multidimensional Indices (MDI)

The function, along with `plot_mdi`, computes and visualise dynamic multidimensional poverty and all associated measures of multidimensional poverty.
...
# index_construction: Index Construction for estimation of Exposure or Sensitivity
 
This function is for conversion of indicators exposure and sensitivity into a vector of index through normalization and weighting. The resulting index from each of the component is then combined via an appropriate model into vulnerability index. The basic usage is:
...
# relative_likert: Convert Likert Data to Relative Scores and knowledge-based Adaptive Capacity
 
This function is for knowledge-based **Adaptive Capacity**. The indices from the various knowledge areas like Awareness, availability, affordability, accessibility, benefits, adequacy, usage, effectiveness, etc can be obtained individually and converted to adaptive capacity. The basic usage is:
...
`Likert` Vector of likert-type factors in the data frame which must be given if the data frame is in text.
...
# odds_summary: Odds-Based Measures for Binary and Categorical Models
...
model An `R` object of estimates from models covered. For now only
